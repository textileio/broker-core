...
// @todo: Handle reload in-progress auctions
...
var (
	log = golog.Logger("market/auction-queue")
	...
	// Structure: /auctions/<auction_id>
	dsQueuePrefix = ds.NewKey("/auctions")

	// dsBidPrefix is the prefix for grouped time-ordered keys used to list bids.
	// Structure: /auctions/<auction_id>/bids/<bid_id>
...
// Handler is called when a request moves from "queued" to "active".
...
func (q *Queue) ListRequests(group string, query Query) ([]openapi.PinStatus, error) {
	...
	results, err := q.store.QueryExtended(dsextensions.QueryExt{
		Query: dsq.Query{
			Prefix:  dsBidPrefix.ChildString(group).String(),
			Filters: filters,
...
func getGroupKey(group, id string) ds.Key {
	return dsBidPrefix.ChildString(group).ChildString(id)
}
